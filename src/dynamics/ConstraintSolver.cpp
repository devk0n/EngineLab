...
void ConstraintSolver::clearConstraints() { m_constraints.clear(); }

void ConstraintSolver::buildJacobian(const std::vector<Particle *> &bodies,
                                     MatrixXd &jacobian,
...
void ConstraintSolver::solveConstrainedSystem(
  ...
  MatrixXd J_M_inv_JT = jacobian * M.asDiagonal().inverse() * jacobian.
                        transpose();
  ...
  accelerations = M.asDiagonal().inverse() * (
                    forces + jacobian.transpose() * lambdas);
